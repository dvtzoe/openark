...
def test_push_rejects_empty_text(registry, store):
    with pytest.raises(ValueError):
        store.push(registry, "defoko", "team", "   ")
...
def test_subscribed_items_merge_with_provenance(registry, store):
    store.subscribe(registry, "observer", "team")
    store.push(registry, "defoko", "team", "User prefers vitest over jest")
    store.push(registry, "defoko", "team", "Deploy target is fly.io")
    ...
    assert all(i["source_agent"] == "defoko" for i in items)
...
def test_subscribed_items_respects_limit(registry, store):
    store.subscribe(registry, "observer", "team")
    store.push(registry, "defoko", "team", "one")
