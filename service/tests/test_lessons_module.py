...
def test_list_empty(registry):
    ...
    assert module.list(registry.agent_home("chiai")) == []
...
def test_reflect_adds_rules_with_provenance(registry):
    ...
    entries = module.list(registry.agent_home("chiai"))
...
def test_reflect_correction_only_uses_correction_source(registry):
    ...
    result = module.reflect(
        registry, "chiai", failures=[], messages=["don't delete without asking"]
    )
...
def test_reflect_nothing_to_reflect(registry):
    module = make_module(FakeRunner())
    result = module.reflect(registry, "chiai", failures=[], messages=[])
    assert result["reason"] == "nothing-to-reflect"


def test_reflect_without_model(registry):
    module = make_module(None)
    result = module.reflect(registry, "chiai", failures=[{"tool": "bash", "summary": "x"}])
...
def test_reflect_dedupes_existing_rules(registry):
    ...
    module.add(registry, "chiai", "Run `make lint` before every commit")
    result = module.reflect(registry, "chiai", failures=[{"tool": "bash", "summary": "x"}])
    assert result["added"] == []
    assert result["skipped"] == 1
    assert result["reason"] == "duplicate"
    assert len(module.list(registry.agent_home("chiai"))) == 1
...
def test_reflect_reinstates_retired_rules(registry):
    ...
    module.retire(registry, "chiai", entry_id)

    result = module.reflect(registry, "chiai", failures=[{"tool": "bash", "summary": "x"}])
    assert len(result["added"]) == 1
    entries = module.list(registry.agent_home("chiai"), active_only=False)
    assert entries[0].status == "active"


def test_add_manual_lesson(registry):
    module = make_module(FakeRunner())
    result = module.add(registry, "chiai", "Always write tests first")
    assert result["added"]["source"] == "manual"
    duplicate = module.add(registry, "chiai", "Always write tests first")
    assert duplicate["reason"] == "duplicate"


def test_retire_unknown_lesson(registry):
    module = make_module(FakeRunner())
    assert module.retire(registry, "chiai", "deadbeef")["reason"] == "not-found"


def test_hits_lifecycle_and_retirement(registry):
    module = make_module(FakeRunner())
    module.add(registry, "chiai", "Keep it simple")
    popular_id = lesson_id("Keep it simple")
    module.add(registry, "chiai", "Never used")
    unused_id = lesson_id("Never used")

    result = module.register_hits(registry, "chiai", [popular_id], "s1")
    assert result["counted"] is True
    assert result["retired"] == []

    again = module.register_hits(registry, "chiai", [popular_id], "s1")
    assert again["counted"] is False

    for i in range(RETIRE_AFTER_STALE_SESSIONS):
        module.register_hits(registry, "chiai", [popular_id], f"s{i + 2}")

    entries = {e.id: e for e in module.list(registry.agent_home("chiai"), active_only=False)}
...
def test_lessons_file_roundtrip(registry, tmp_path):
    ...
    path = registry.agent_home("chiai") / "lessons.md"
    parsed = _parse(path)
