...
EVOLVING = """# chiai — learned preferences

Managed by chiai. Every change is appended to logs/audit.log.
...
def registry(tmp_path):
    ...
    reg.ensure_layout()
    reg.create("chiai")
    reg.write_persona("chiai", "# core", EVOLVING)
    return reg
...
def test_evolve_below_threshold_is_rejected(registry):
    ...
    result = module.evolve(registry, "chiai", ["one signal", "one signal", "  "])
...
def test_evolve_without_model_is_noop(registry):
    ...
    result = module.evolve(registry, "chiai", ["a", "b", "c"])
    assert result["updated"] is False
...
def test_evolve_applies_adds_and_replaces(registry):
    ...
    _, evolving = registry.read_persona("chiai")
    ...
    assert "Managed by chiai" in evolving
...
def test_threshold_counts_distinct_signals_only(registry):
    module = make_module(FakeRunner())
    result = module.evolve(registry, "chiai", ["same", "same", "same"])
    assert result["reason"] == "below-threshold"
...
def test_bundled_chiai_persona_seeds_agent(tmp_path):
    reg = AgentRegistry(tmp_path)
    reg.ensure_layout()
    assert "chiai" in reg.bundled_personas()
    reg.create("mascot", persona="chiai")
    core, evolving = reg.read_persona("mascot")
    assert "chiai" in core
