...
def test_distill_creates_draft(registry):
    ...
    result = module.distill(registry, "chiai", "ran make lint, make test, tagged v1.0")
    assert result["reason"] is None
    assert result["created"]["name"] == "release-checklist"
    assert result["created"]["status"] == "draft"

    home = registry.agent_home("chiai")
...
def test_verify_moves_draft_to_skills(registry):
    ...
    module.distill(registry, "chiai", "some successful trace")
    result = module.verify(registry, "chiai", "release-checklist")
...
def test_verify_unknown_slug(registry):
    module = make_module(FakeRunner())
    assert module.verify(registry, "chiai", "ghost")["reason"] == "not-found"
    assert module.verify(registry, "chiai", "../escape")["reason"] == "invalid-slug"


def test_distill_empty_trace(registry):
    module = make_module(FakeRunner())
    assert module.distill(registry, "chiai", "   ")["reason"] == "empty-trace"


def test_distill_without_model(registry):
    module = make_module(None)
    assert module.distill(registry, "chiai", "trace")["reason"] == "no-model"


def test_distill_unparseable_output(registry):
    module = make_module(FakeRunner(output="This is not a skill format"))
    assert module.distill(registry, "chiai", "trace")["reason"] == "unparseable"


def test_distill_llm_error(registry):
    module = make_module(FakeRunner(fail=RuntimeError("boom")))
    assert module.distill(registry, "chiai", "trace")["reason"] == "llm-error"


def test_distill_slug_collisions_get_suffixes(registry):
    module = make_module(FakeRunner())
    module.distill(registry, "chiai", "trace one")
    result = module.distill(registry, "chiai", "trace two")
...
def test_distill_sanitizes_bad_names(registry):
    ...
    result = module.distill(registry, "chiai", "trace")
...
def test_verify_twice_conflicts(registry):
    module = make_module(FakeRunner())
    module.distill(registry, "chiai", "trace one")
    module.verify(registry, "chiai", "release-checklist")
    module.distill(registry, "chiai", "trace two")
    module.verify(registry, "chiai", "release-checklist-2")
    result = module.verify(registry, "chiai", "release-checklist-2")
...
def test_compositional_prompt_lists_existing_skills(registry):
    ...
    module2.distill(registry, "chiai", "trace two")
