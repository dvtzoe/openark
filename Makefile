...
fmt: ## auto-format everything
	cd $(PLUGIN_DIR) && bunx biome format --write src tests *.json
...
plugin-lint:
	cd $(PLUGIN_DIR) && bunx biome check src tests
...
gen-types: ## regenerate plugin TS types from the service OpenAPI spec
	...
	cd $(PLUGIN_DIR) && bunx openapi-typescript /tmp/openark-openapi.json -o src/generated/api-types.ts
...
create-module: ## scaffold a new module: make create-module name=foo
	bun scripts/create_module.mjs $(name)
